...
public class SimilarityService {
    ...
    public SimilarityService() throws Exception {
    ...
    public static void main(String[] args) throws Exception {
        ...
        getSimilarities(referenceWord, similarityService);

        similarityService.wordNetSimilarities("dog", "soul");

        System.out.println(similarityService.getLevenshteiniSimilarity("cat", "dog"));
        System.out.println(similarityService.getLevenshteiniSimilarity("cat", "at"));
    }

    private static void getSimilarities(String referenceWord, SimilarityService similarityService) throws IOException {
        try (BufferedReader reader = new BufferedReader(new FileReader("/home/enghin/Documents/Tasks/domainsUK.txt"))) {
            String line = reader.readLine();
            while (line != null) {
                line = line.trim();
                System.out.println(line);
                wordNetSim.put(line, similarityService.getDomainsWordNetSimilarity(referenceWord, line));
                lSim.put(line, similarityService.getLevenshteiniSimilarity(referenceWord, line));
                embeddingSim.put(line, similarityService.getDomainsEmbeddingsSimilarity(referenceWord, line));
                line = reader.readLine();
            }
        }
        wordNetSim = similarityService.sortByValue(wordNetSim);
        lSim = similarityService.sortByValue(lSim);
        embeddingSim = similarityService.sortByValue(embeddingSim);
    }
}
